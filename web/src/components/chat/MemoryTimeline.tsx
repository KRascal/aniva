...
export function MemoryTimeline({ characterId }: MemoryTimelineProps) {
  ...
  const formatDate = (dateStr: string): string => {
    ...
    return year === currentYear
      ? t('dateMonthDay', { month, day })
      : t('dateYearMonthDay', { year, month, day });
  };
    ...
    <div className="px-1 pb-4">
      ...
      <div className="flex gap-2 mb-4 px-2">
        <div className="flex-1 bg-white/5 rounded-xl px-3 py-2 text-center border border-white/8">
          ...
          <div className="text-gray-500 text-[10px] mt-0.5">{t('bondLevelLabel')}</div>
        </div>
        <div className="flex-1 bg-white/5 rounded-xl px-3 py-2 text-center border border-white/8">
          <div className="text-purple-300 font-bold text-base">{data.totalMessages}</div>
          <div className="text-gray-500 text-[10px] mt-0.5">{t('conversationCount')}</div>
        </div>
        {data.firstMessageAt && (
          <div className="flex-1 bg-white/5 rounded-xl px-3 py-2 text-center border border-white/8">
            <div className="text-purple-300 font-bold text-xs">{formatDate(data.firstMessageAt)}</div>
            <div className="text-gray-500 text-[10px] mt-0.5">{t('firstConversationDate')}</div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        {data.memories.map((ep, idx) => (
          ...
          >
            ...
            {/* 重要度インジケーター */}
            {ep.importance >= 4 && (
              <div className="flex-shrink-0 text-yellow-400 text-xs mt-0.5" title={t('importanceLevel', { level: ep.importance })}>
                ⭐
              </div>
            )}
          </div>
