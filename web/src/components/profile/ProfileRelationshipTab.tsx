...
export interface ProfileRelationshipTabProps {
...
export function ProfileRelationshipTab({
  character,
  relationship,
  level,
  xp,
    ...
    <div className="space-y-5 pt-2 pb-24">
      ...
      <div className="bg-gray-900/60 rounded-2xl p-4 border border-white/5 space-y-4">
        ...
        <button
          ...
          className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 ${
            saveSuccess ? 'bg-green-600/80 text-white' : 'bg-purple-600 hover:bg-purple-700 text-white'
            ...
            <>
              {ranking.slice(0, 3).map((entry) => {
                ...
                return (
                  <div
                    key={entry.rank}
                    className={`flex items-center gap-3 px-4 py-3 border-b border-white/5 last:border-0 ${entry.isMe ? 'bg-purple-900/30' : ''}`}
                  >
                ...
                <div className="px-4 py-3 bg-orange-950/20 border-t border-orange-500/20">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs mt-0.5">まだ{character?.name ?? 'キャラ'}に追いつけるかも…もっと話しかけてみよう！</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      {character && (() => {
        ...
        if (character.personalityTraits?.length) {
          ...
          if (traits.length) dbProfile['性格'] = traits.slice(0, 3).map((t: string | PersonalityTrait) => typeof t === 'string' ? t : t.trait).join('、');
        }
        const profileEntries = hardcoded
          ? (Object.entries(hardcoded) as [keyof typeof CHARACTER_PROFILES[string], string][])
              .filter(([, v]) => !!v)
              .map(([key, value]) => ({ label: PROFILE_LABELS[key as keyof typeof PROFILE_LABELS], value }))
        ...
        return (
          <div className="bg-gray-900/80 rounded-2xl p-5 border border-white/10 shadow-lg">
            <p className="text-purple-400 text-xs font-semibold uppercase tracking-widest mb-4">プロフィール</p>
      ...
      {character?.description && (
        <div className="bg-gray-900/80 rounded-2xl p-5 border border-white/10 shadow-lg">
          <p className="text-gray-400 text-xs font-semibold uppercase tracking-widest mb-3">紹介</p>
      ...
      {character && CREW_MEMBERS[character.slug] && (
        <div>
          <p className="text-gray-400 text-xs font-semibold uppercase tracking-widest mb-3 px-1">関連キャラクター</p>
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide" style={{ scrollbarWidth: 'none' }}>
            {CREW_MEMBERS[character.slug].map((member) => (
              <div key={member.name} className="flex-shrink-0 bg-gray-900/80 rounded-xl p-3 border border-white/5 text-center w-24">
      ...
      {character?.personalityTraits && Array.isArray(character.personalityTraits) && character.personalityTraits.length > 0 && (() => {
        ...
        const converted: PersonalityTrait[] = rawTraits.map((t, i) => {
          if (typeof t === 'object' && t !== null && 'trait' in t && 'value' in t) return t as PersonalityTrait;
      ...
      {catchphrases.length > 0 && (
        <div>
          <p className="text-gray-400 text-xs font-semibold uppercase tracking-widest mb-3 px-1">名言</p>
          <div className="space-y-3">
            {catchphrases.map((phrase, i) => (
              <div key={i} className="bg-gray-900/70 rounded-xl p-4 border border-purple-900/30 relative overflow-hidden">
          ...
          {[
            {
              ...
              value: (relationship?.totalMessages ?? 0).toLocaleString(),
              label: 'メッセージ',
            },
            {
              ...
              value: relationship?.firstMessageAt
            ...
            },
            {
              ...
              value: relationship?.lastMessageAt
                ? new Date(relationship.lastMessageAt).toLocaleDateString('ja-JP', { month: 'short', day: 'numeric' })
                : '—',
              label: '最後の会話',
            },
          ].map(({ icon, value, label }) => (
            <div key={label} className="bg-gray-900/80 rounded-2xl p-4 border border-white/5 text-center">
              <div className="flex justify-center mb-1.5">
                <svg className="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>{icon}</svg>
              </div>
              ...
              <div key={m.id} className="relative flex items-start gap-4 pl-2">
                ...
                <div
                  className={`flex-1 min-w-0 rounded-xl p-3.5 border transition-all ${
                    m.achieved ? 'bg-purple-900/20 border-purple-500/30' : 'bg-gray-900/40 border-gray-800/50 opacity-50'
                  }`}
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`font-semibold text-sm ${m.achieved ? 'text-white' : 'text-gray-500'}`}>{m.title}</span>
                    {m.achieved ? (
                      <span className="text-xs text-purple-300 bg-purple-900/60 px-2 py-0.5 rounded-full">✓ 達成</span>
                    ) : (
                      <span className="text-xs text-gray-600 bg-gray-800/60 px-2 py-0.5 rounded-full">Lv.{m.level}で解放</span>
                    )}
                  </div>
