...
import type { DlContent } from '@/app/profile/[characterId]/profile-data';

export interface ProfileDlTabProps {
  dlContents: DlContent[];
  dlLoading: boolean;
  onFcTabClick: () => void;
}

export function ProfileDlTab({ dlContents, dlLoading, onFcTabClick }: ProfileDlTabProps) {
  return (
          ...
          {dlContents.map((item) => (
            <div
              ...
              className={`relative rounded-2xl overflow-hidden border ${
                item.locked ? 'border-gray-700/50 bg-gray-900/60' : 'border-purple-500/30 bg-gray-900/80'
              }`}
            >
              {item.thumbnailUrl ? (
