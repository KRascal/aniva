        ...
        <div className="bg-white/[0.04] backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-[0_0_60px_rgba(168,85,247,0.15)]">
          ...
          {step === 'email' && (
            <>
              ...
              <p className="text-xs text-gray-500 text-center mt-3">クレジットカード不要・新規/既存を自動判定</p>

              <p className="mt-6 text-center text-sm text-gray-500">
                すでにアカウントをお持ちの方は{' '}
                <Link href={`/login${inviteCode ? `?code=${inviteCode}` : ''}`} className="text-purple-400 hover:text-purple-300 underline">
                  ログイン
                </Link>
              </p>
            </>
