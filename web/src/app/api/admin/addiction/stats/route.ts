...
export async function GET() {
  ...
  const past7Dates = past7Days.map((d) => new Date(d));
  ...
  return NextResponse.json({
    ...
    dailyEvents: {
      ...
      past7Days: past7DayEvents.map((e) => ({ date: e.date.toISOString().slice(0, 10), type: e.eventType, count: e._count })),
