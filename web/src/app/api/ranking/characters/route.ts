...
/** コインランキングをDBから取得 */
async function fetchCoinsRanking(period: string, limit: number): Promise<RankingEntry[]> {
...
/** メッセージ数ランキングをDBから取得 */
async function fetchMessagesRanking(period: string, limit: number): Promise<RankingEntry[]> {
  const periodFrom = getPeriodFilter(period);
...
export async function GET(req: NextRequest) {
  ...
  if (type === 'coins') {
    const ranking = await cacheGet(
      CACHE_KEYS.ranking(type, period),
      CACHE_TTL.RANKING,
      () => fetchCoinsRanking(period, limit),
    );
  ...
  if (type === 'messages') {
    const ranking = await cacheGet(
      CACHE_KEYS.ranking(type, period),
      CACHE_TTL.RANKING,
      () => fetchMessagesRanking(period, limit),
    );
    return NextResponse.json({ type, period, ranking });
  }
