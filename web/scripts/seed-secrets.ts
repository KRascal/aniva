/**
 * seed-secrets.ts
 * Migrate hardcoded CHARACTER_SECRETS to the SecretContent DB table
 * Run: DATABASE_URL="..." npx tsx scripts/seed-secrets.ts
 */
...
const CHARACTER_SECRETS: Record<string, {
  unlockLevel: number;
...
async function main() {
  console.log('🌱 Seeding secret contents...');

  let totalInserted = 0;
  let totalSkipped = 0;

  for (const [slug, secrets] of Object.entries(CHARACTER_SECRETS)) {
    ...
    if (!character) {
      console.log(`  ⚠️  Character slug="${slug}" not found in DB, skipping`);
      continue;
    }

    console.log(`  📝 Processing ${character.name} (${slug}): ${secrets.length} secrets`);

    for (let i = 0; i < secrets.length; i++) {
      const s = secrets[i];

      // Check if already exists (by characterId + title)
      const existing = await prisma.secretContent.findFirst({
        where: { characterId: character.id, title: s.title },
      });

      if (existing) {
        console.log(`    ✓ Already exists: "${s.title}" — skipping`);
        totalSkipped++;
        continue;
      }
      ...
      console.log(`    ✅ Inserted: "${s.title}" (Lv.${s.unlockLevel})`);
      totalInserted++;
    }
  }
